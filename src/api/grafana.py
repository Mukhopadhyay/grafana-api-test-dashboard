...
from modules.grafana import users, organization
...
async def create_new_user(model: grafana_schemas.CreateUser, response: Response):
    try:
        ...
        return grafana_schemas.ErrorResponse(
            message=err.message, response=err.data, status=err.status_code
        )
    else:
        return resp
...
async def create_organization(model: grafana_schemas.CreateOrg, response: Response):
    try:
        resp = await organization.create_organization(
            model.name, model.address.address1, model.address.address2,
